...
- Keep graph-widget callbacks free of live `RefCell` borrows before routing events back into shell/context state; selection callbacks can synchronously trigger `set_scene()` and will panic if the graph scene borrow is still held.
