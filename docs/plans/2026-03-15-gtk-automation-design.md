...
Capture the approved design for `COMP-GTK-AUTOMATION-008` before implementation details drift between GTK, later `knotd` integration work, and the parity harnesses.

## Locked Decisions

- Automation is semantic and daemon-consumable, not widget-addressed.
- Later `knotd` work must be able to query GTK state and dispatch semantic UI actions through a typed local automation contract.
...
- This slice also does not include live `knotd` IPC/RPC transport wiring; daemon-style access is covered through mocked transport harnesses.
...
- The protocol must be explicit enough that later `knotd` updates can be implemented from local docs without inferring schemas from GTK internals.
